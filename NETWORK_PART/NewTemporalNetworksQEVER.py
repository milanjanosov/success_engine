...
def get_networks():
    ...
    directors_s = directors_s.intersection(QEVER)
   ...
   # dirrrrs = ['nm0000184', 'nm0000245']
    ...
    folderout = 'NEWTemporal/3_edgelists_QEVER/'
    if not os.path.exists(folderout): os.makedirs(folderout)

    gout = open('NEWTemporal/3_edgelists_QEVER/networks_size_over_time_QEVER.dat', 'w')
    ...
    for year, edges in edges_jacc.items():
        ...
        fout = open(edgefolder + '/'+str(year)+'_edge_list_jaccard_QEVER.dat', 'w')
    ...
    for year, edges in edges_cnt.items():
        ...
        fout = open(edgefolder + '/'+str(year)+'_edge_list_count_QEVER.dat', 'w')
        for edge, weight in edges.items():
            fout.write(edge + '\t' + str(weight) + '\n')
        fout.close()
...
def calc_centr(years):
    ...
    for year in years:
        ...
        infile  = 'NEWTemporal/3_edgelists_QEVER/' + str(year) + '/'+str(year)+'_edge_list_jaccard_QEVER.dat'
        G_ig    = Graph.Read_Ncol(infile, weights = True, directed=False)
        outfile = 'NEWTemporal/3_edgelists_QEVER/' + str(year) + '/'+str(year)+'_centralities_jaccard_QEVER.dat'
...
def get_network_centralities():
    ...
    edgefolder = 'NEWTemporal/3_edgelists_QEVER/'
...
def get_centrality_careers(top_directors):
    ...
    for year in years:
        ...
        for ind, line in enumerate(open(edgefolder + year + '/' + year + '_centralities_jaccard_QEVER.dat')):
            #if ind == 3: break
            ...
            if 'degree_' not in line:
                ...
                name,degree_ig,clustering_ig,pagerank_ig,betweennesse_ig,constraint_ig = line.strip().split(',')
                ...
                if name in directors:
                    ...
                    measures = [degree_ig,clustering_ig,pagerank_ig,betweennesse_ig,constraint_ig]
                    names    = ['degree', 'clustering', 'pagerank', 'betweenness', 'constraint']
    ...
    folderout = 'NEWTemporal/4_directors_centralities_QEVER/'
    if not os.path.exists(folderout): os.makedirs(folderout)
