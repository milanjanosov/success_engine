...
class SimpleCareerTrajectory:
    ...
    def __init__(self, name, inputfile, impactm):
        ...
        self.events  = [tuple([line.strip().split('\t')[0],float(line.strip().split('\t')[1]),float(line.strip().split('\t')[impactm + 2])]) for line in gzip.open(inputfile) if 'year' not in line and 'None' not in line.strip().split('\t')[impactm+ 2]]  
